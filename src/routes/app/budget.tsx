...
import { CategoryDot, DataList, HeroMetric, Kicker } from '#/components/dense'
...
function BudgetPage() {
  ...
  useEffect(() => {
    if (!data) return
    setFlexBudget(
      data.flexBudget != null
        ? String(data.flexBudget)
        : String(data.totals.flexPlanned || ''),
    )
    setExpectedIncome(
          ...
          <Button
            ...
            onClick={() =>
              void copyFromPrevious({ month }).then((id) =>
                toast.success(
                  id ? 'Copied from last month' : 'No previous budget',
                ),
              )
            }
          >
        ...
        <Page>
          ...
          <PageBody>
            <Panel
              ...
              tone={
                rollup.flexPlanned > 0 &&
                data.totals.flexSpent > rollup.flexPlanned
                  ? 'over'
                  : 'default'
              }
              action={
            ...
            <Panel
              ...
              tone={
                rollup.fixedPlanned > 0 &&
                rollup.fixedSpent > rollup.fixedPlanned
                  ? 'over'
                  : 'default'
              }
              flush
